...
print("INFO: Starting bot initialization...") # Added print
TOKEN = os.environ.get("BOT_TOKEN")
if not TOKEN:
    print("CRITICAL: BOT_TOKEN is not set in Secrets. Exiting.") # Modified print
...
print("INFO: Flask app and TeleBot instance created.") # Added print
...
try:
    ...
    print(f"INFO: Loaded {len(all_words)} words from words.txt.") # Added print
except FileNotFoundError:
    all_words = ["чат", "дзвінок", "підтримка", "запит", "email"]
    print("WARNING: words.txt not found. Using default words.") # Modified print
...
def finish_game(chat_id: Optional[int] = None, silent: bool = False):
    print(f"INFO: Finishing game in chat {chat_id}, silent={silent}.") # Added print
    ...
    if not silent and target_chat_id != 0 and any(teams_score.values()):
        ...
        if teams.get(winner):
            for uid in teams[winner]:
                try:
                    ...
                    summary += f"- User {uid}\n"
        try: # Added try-except for send_message
            bot.send_message(target_chat_id, summary, parse_mode="Markdown")
        except ApiTelegramException as e:
            print(f"ERROR: Could not send game summary to {target_chat_id}: {e}") # Added print
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send new game prompt to {target_chat_id}: {e}") # Added print
    print("INFO: Game state reset.") # Added print
...
def show_score():
    print("INFO: Displaying score.") # Added print
    ...
    if current_chat_id != 0:
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send score to {current_chat_id}: {e}") # Added print
def update_timer_thread(user_id: int, start_time: float):
    print(f"INFO: Timer thread started for user {user_id}.") # Added print
    ...
    print(f"INFO: Timer thread finished for user {user_id}.") # Added print

def send_word_to_player(user_id: int, is_initial: bool = False):
    print(f"INFO: Sending word to player {user_id}, initial={is_initial}.") # Added print
    state = player_states.get(user_id)
    if not state:
        print(f"WARNING: No player state for user {user_id}.") # Added print
        return
    ...
    try:
        ...
        print(f"INFO: Sent photo word '{word}' to {user_id}.") # Added print
    except (FileNotFoundError, ApiTelegramException) as e: # Catch exception for better logging
        print(f"WARNING: Photo for '{word}' not found or API error: {e}. Sending as text.") # Modified print
        try:
            if is_initial:
            ...
            print(f"INFO: Sent text word '{word}' to {user_id}.") # Added print
        except ApiTelegramException as e:
            print(f"ERROR: Failed to send word to user {user_id}: {e}") # Added print
...
def end_round(user_id: int, score: int):
    print(f"INFO: Ending round for user {user_id} with score {score}.") # Added print
    global round_in_progress, active_player_id, current_turn_index, group_timer_message_id
    if not round_in_progress:
        print("WARNING: end_round called but no round in progress.") # Added print
        return
    ...
    if current_chat_id != 0:
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send round result to {current_chat_id}: {e}") # Added print
        ...
        if is_circle_complete:
            ...
            try: # Added try-except for send_message
                ...
                print(f"ERROR: Could not send circle end message to {current_chat_id}: {e}") # Added print
        else:
            ...
            try: # Added try-except for send_message
                ...
                print(f"ERROR: Could not send next team prompt to {current_chat_id}: {e}") # Added print
def start_round_for_player(user_id: int):
    print(f"INFO: Starting round for player {user_id}.") # Added print
    ...
    if not available_words:
        if current_chat_id != 0:
            try: # Added try-except for send_message
                ...
                print(f"ERROR: Could not send 'words ended' message to {current_chat_id}: {e}") # Added print
        finish_game(); return
...
def setup_command(message: types.Message):
    print(f"INFO: Received /setup command from user {message.from_user.id} in chat {message.chat.id}") # Added print
    ...
    try: # Added try-except for send_message
        ...
        print(f"ERROR: Could not send setup prompt to {message.chat.id}: {e}") # Added print
def process_team_count(message: types.Message):
    print(f"INFO: Processing team count from user {message.from_user.id}.") # Added print
    try:
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send team name prompt to {message.chat.id}: {e}") # Added print
    except (ValueError, TypeError) as e: # Catch exception for better logging
        print(f"ERROR: Invalid team count input from {message.from_user.id}: {e}") # Added print
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send retry message for team count to {message.chat.id}: {e}") # Added print
def process_team_name(message: types.Message, current_num: int, total_teams: int, collected_names: List[str]):
    print(f"INFO: Processing team name {current_num}/{total_teams} from user {message.from_user.id}.") # Added print
    if not message.text:
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'text name' prompt to {message.chat.id}: {e}") # Added print
    ...
    if not team_name:
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'empty name' prompt to {message.chat.id}: {e}") # Added print
    ...
    if current_num < total_teams:
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send next team name prompt to {message.chat.id}: {e}") # Added print
    else:
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send teams configured message to {message.chat.id}: {e}") # Added print

@bot.message_handler(commands=["start"])
def start(message: types.Message):
    print(f"INFO: Received /start command from user {message.from_user.id} in chat {message.chat.id}") # Added print
    global current_chat_id; current_chat_id = message.chat.id
    if not teams:
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'no teams' message to {message.chat.id}: {e}") # Added print
    ...
    try: # Added try-except for send_message
        ...
        print(f"ERROR: Could not send rules message to {current_chat_id}: {e}") # Added print
    markup = types.InlineKeyboardMarkup()
    ...
    try: # Added try-except for send_message
        ...
        print(f"ERROR: Could not send team selection message to {current_chat_id}: {e}") # Added print

@bot.callback_query_handler(func=lambda call: call.data.startswith("team_"))
def join_team(call: types.CallbackQuery):
    print(f"INFO: Received join_team callback from user {call.from_user.id} for team {call.data}.") # Added print
    ...
    for name in teams_order:
        ...
        for m_id in members:
            try:
                ...
                print(f"WARNING: Could not get chat info for user {m_id}: {e}") # Added print
    ...
    except ApiTelegramException as e: # Catch exception for better logging
        print(f"ERROR: Could not edit message after join_team for user {uid}: {e}") # Added print
        pass # The original code had a bare pass, keeping it here after logging
    if not game_active:
        markup = types.InlineKeyboardMarkup()
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'start game' prompt to {call.message.chat.id}: {e}") # Added print

def start_round_handler(message_or_call: types.Message | types.CallbackQuery):
    print(f"INFO: Received start_round_handler from user {message_or_call.from_user.id}.") # Added print
    global game_active, round_in_progress, active_player_id, available_words, current_turn_index, group_timer_message_id
    user = message_or_call.from_user
    if not user:
        print("WARNING: start_round_handler called without user info.") # Added print
        return
    chat_id = message_or_call.message.chat.id if isinstance(message_or_call, types.CallbackQuery) else message_or_call.chat.id
    if not chat_id:
        print("WARNING: start_round_handler called without chat ID.") # Added print
        return
    ...
    if round_in_progress:
        ...
        print("WARNING: Round already in progress.") # Added print
        return
    if not teams:
        try: # Added try-except for send_message
            return bot.send_message(chat_id, "Спочатку налаштуйте команди за допомогою /setup")
        except ApiTelegramException as e:
            print(f"ERROR: Could not send 'no teams' message (start_round_handler) to {chat_id}: {e}") # Added print
            return
    ...
    if not player_team:
        ...
        print(f"WARNING: User {user.id} not in a team.") # Added print
        return
    if player_team != expected_team and game_active:
        ...
        print(f"WARNING: User {user.id} from team {player_team} tried to start, but it's {expected_team}'s turn.") # Added print
        return
    if not game_active:
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'game started' message to {current_chat_id}: {e}") # Added print
    timer_msg = None # Initialize to None
    try: # Added try-except for send_message
        timer_msg = bot.send_message(current_chat_id, f"⏳ Залишилось часу: *{ROUND_TIME}* сек", parse_mode="Markdown")
    except ApiTelegramException as e:
        print(f"ERROR: Could not send timer message to {current_chat_id}: {e}") # Added print
    ...
    try:
        ...
        print(f"INFO: Notified chat about active player @{username}.") # Added print
    except Exception as e: # Catch all exceptions for robustness
        print(f"ERROR: Failed to notify chat about active player: {e}") # Added print
        pass # The original code had a bare pass, keeping it here after logging
...
def handle_start_round_callback(call: types.CallbackQuery):
    print(f"INFO: Callback 'start_game' from user {call.from_user.id}.") # Added print
...
def handle_response(call: types.CallbackQuery):
    print(f"INFO: Received word response callback '{call.data}' from user {call.from_user.id}.") # Added print
    uid = call.from_user.id
    if not round_in_progress or uid != active_player_id:
        bot.answer_callback_query(call.id, "⏳ Зачекай свою чергу")
        print(f"WARNING: User {uid} tried to respond out of turn or round not in progress.") # Added print
        return
    state = player_states.get(uid)
    if not state:
        bot.answer_callback_query(call.id, "Помилка: не знайдено стан гри.")
        print(f"ERROR: No game state found for user {uid} during response handling.") # Added print
        return
    if call.data == "right":
        state["score"] += 1;
        bot.answer_callback_query(call.id, "✅ +1 бал")
        print(f"INFO: User {uid} got word right. Score: {state['score']}.") # Added print
    else:
        bot.answer_callback_query(call.id, "⏭️ Наступне слово")
        print(f"INFO: User {uid} skipped or got word wrong.") # Added print

    if state["word_count"] >= ROUND_LIMIT or not available_words:
        print(f"INFO: Round limit reached or no available words. Ending round for user {uid}.") # Added print
...
def handle_new_circle(call: types.CallbackQuery):
    print(f"INFO: Received new_circle callback from user {call.from_user.id}.") # Added print
    ...
    if not teams_order:
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'no teams' message (new_circle) to {call.message.chat.id}: {e}") # Added print
            return
    next_team = teams_order[current_turn_index]
    display_next_team = _get_team_display_name(next_team)
    markup = types.InlineKeyboardMarkup()
    ...
    try:
        ...
        print(f"WARNING: Could not edit 'new_circle' message in chat {call.message.chat.id}: {e}") # Modified print
    try:
        ...
        print(f"ERROR: Could not send 'next circle' prompt to {call.message.chat.id}: {e}") # Added print

@bot.callback_query_handler(func=lambda call: call.data == "finish_game")
def callback_finish_game(call: types.CallbackQuery):
    print(f"INFO: Received finish_game callback from user {call.from_user.id}.") # Added print
...
def handle_setup_new_game(call: types.CallbackQuery):
    print(f"INFO: Received setup_new_game callback from user {call.from_user.id}.") # Added print
    bot.answer_callback_query(call.id)
    if isinstance(call.message, types.Message):
        ...
        try: # Added try-except for send_message
            ...
            print(f"ERROR: Could not send 'setup failed' message to user {call.from_user.id}: {e}") # Added print
...
def webhook():
    ...
    print("INFO: Webhook endpoint received a request.") # Added print
    if flask.request.headers.get('content-type') == 'application/json':
        json_string = flask.request.get_data().decode('utf-8')
        update = telebot.types.Update.de_json(json_string)
        print(f"INFO: Received update from Telegram: {update.update_id}") # Added print
        if update:
            try: # Added try-except for process_new_updates
                bot.process_new_updates([update])
                print(f"INFO: Successfully processed update {update.update_id}.") # Added print
            except Exception as e:
                print(f"CRITICAL ERROR: Failed to process update {update.update_id}: {e}") # Added print
                import traceback
                print(traceback.format_exc()) # Print full traceback
        return '', 200
    else:
        print(f"WARNING: Webhook received non-JSON request with content-type: {flask.request.headers.get('content-type')}") # Added print
        flask.abort(403)
...
if __name__ != "__main__":
    print("INFO: Starting production mode initialization (Gunicorn).") # Added print
    if not TOKEN:
...
if __name__ == "__main__":
    print("INFO: Running in local development mode (Polling).") # Added print
    bot.remove_webhook()
